...
CREATE TABLE IF NOT EXISTS `db_brecho`.`tbl_user` (
  ...
  `user_phone_country_code` VARCHAR(6) NOT NULL,
  `user_phone_state_code` VARCHAR(4) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `db_brecho`.`tbl_seller` (
  ...
  `seller_bank_agency` VARCHAR(50) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `db_brecho`.`tbl_generate` (
  ...
  CONSTRAINT GENERATE_CLIENT_FK FOREIGN KEY (`client_id`) REFERENCES `tbl_client`(`client_id`) DELETE RESTRICT ON UPDATE RESTRICT
)ENGINE = InnoDB;
...
-- -----------------------------------------------------
-- Table `db_brecho`.`tbl_tracking`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `db_brecho`.`tbl_tracking` (
  `tracking_id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `tracking_name` ENUM('pending', 'shipped', 'in_transit', 'delivered', 'returned') NOT NULL DEFAULT 'pending',
